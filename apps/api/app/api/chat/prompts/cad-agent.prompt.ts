...
// EVAL(diagnose-before-switching): pending benchmark-2026-04-20 — replaces "stop after 1-2 retries" with claude-code's diagnose-before-switching guidance in <error_handling> (audit R2). Validates that the agent stops blindly retrying identical actions but does not abandon viable approaches after a single failure.
// EVAL(faithful-reporting): pending benchmark-2026-04-20 — adds faithful-reporting bullet to <constraints> (audit R5). Validates that the agent stops claiming "all tests pass" when output shows failures and does not characterise incomplete work as done.
// EVAL(tool-usage-policy): pending benchmark-2026-04-20 — new <tool_usage_policy> static section codifies parallel-vs-sequential calls and bans placeholder values (audit R6). Validates fewer wasted sequential reads and zero placeholder-arg tool calls.
// EVAL(screenshot-cap): pending benchmark-2026-04-20 — caps screenshots at 2 per inspection cycle and bans chaining a single screenshot after multi_angle in <visual_inspection> (audit R7). Validates lower screenshot tool-call counts per task.
// EVAL(top-level-export-positive): pending benchmark-2026-04-20 — replaces the OpenSCAD-only "do not add module-only library files" warning with a kernel-aware <test_requirements> directive that pushes the agent to ADD a top-level export (via KernelConfig.topLevelExportExample) rather than skip a test (audit R8 follow-up). The block now states the requirement once with a single canonical example; the imperative "Add ..." recovery sentence is reserved for the test_model tool error so the system prompt no longer dual-states. Validates that the agent stops shrinking test.json on missing-export errors and instead patches the source file with edit_file. Follow-up 2026-04-21: removed the test_model tool description pre-warning that duplicated this block, single-source-of-truth fix.
...
// EVAL(tone): pending benchmark-2026-04-20 — new <tone> static section (objectivity, no time estimates, no colon-before-tool-call, no emojis) (audit R9). Validates a measurable drop in flattery/filler tokens per turn.
// EVAL(plan-mode-strict): pending benchmark-2026-04-20 — tightens <plan_mode> to forbid non-readonly tool calls except .plan.md edit (audit R10), mirroring claude-code's system-reminder-plan-mode-is-active-iterative.md. Validates zero non-readonly tool calls during plan mode.
// EVAL(workflow-decompose): pending benchmark-2026-04-20 — prepends workflow step 0 (Decompose) for multi-component models with a skip-when-trivial escape hatch (audit R12). Validates fewer dropped components in benchmark fixtures with reference images / BOMs.
// EVAL(iterate-on-defect): pending benchmark-2026-04-20 — rewrites workflow inspect step to require re-render on any defect found and continue iterating until no defects remain (audit R13). Validates fewer single-render-then-done failures.
// EVAL(self-grounded-verification): pending benchmark-2026-04-20 — prepends "predict expected properties before screenshot" guidance to <visual_inspection> (audit R14). Validates fewer "looks right"-style false positives by anchoring screenshot review against an explicit prediction.
// EVAL(system-rules): pending benchmark-2026-04-20 — new <system_rules> static section codifies no-identical-retry on permission denial (R16) and bans URL hallucination (R18). Validates zero retries after denial errors and zero invented URLs in citations.
// EVAL(safety): pending benchmark-2026-04-20 — new condensed <safety> static section codifies destructive-action confirmation for delete_file, export overwrite, and mount-path mutation (audit R17). Validates zero un-confirmed destructive operations.
// EVAL(when-not-to-use-trim): pending benchmark-2026-04-21 — removes universal "When NOT to use" sections from 11 tool descriptions (6 dropped entirely, 5 collapsed to a single positive trailing redirect), keeps trimmed single-bullet form on test_model + edit_tests only (audit R19 revised). Validates no regression in tool-selection accuracy on tool-use,smoke benchmarks, with measurable static-prompt token reduction. New context-engineering-policy `Negative Guidance Is Selective` rule codifies the ratio (≤ 20% of toolbelt).
...
export async function getCadSystemPrompt(
  ...
  options: {
    ...
    gitStatus?: string;
    /**
     * Per-section telemetry hook (R23). Invoked once per non-empty section as
