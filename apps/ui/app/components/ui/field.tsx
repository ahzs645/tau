...
function FieldLegend({
  ...
  variant = 'legend',
  ...props
}: React.ComponentProps<'legend'> & { readonly variant?: 'legend' | 'label' }): React.JSX.Element {
  return (
    <legend
...
function FieldError({
  ...
  const content = useMemo(() => {
    if (children) {
      // oxlint-disable-next-line react/jsx-no-useless-fragment -- Fragment wrapper satisfies the React.JSX.Element return type (children can be a Promise)
      return <>{children}</>;
    }
    ...
    return (
      <ul className='ml-4 flex list-disc flex-col gap-1'>
        {/* oxlint-disable-next-line react/no-array-index-key -- index is used to generate a unique key */}
