...
The kernel machine dynamically creates workers from a `KernelConfig` array injected at startup. Each entry bundles an `id`, `url`, and optional `options`. Array position defines `canHandle` priority (first match wins).

### First-party kernels

For kernels shipped with Tau, add an entry to `defaultKernelConfig` in `apps/ui/app/constants/kernel-workers.ts`:
...
<BuildProvider buildId={id} kernelConfig={extendedConfig}>
```
...
Middleware is **dynamically loaded** in the Web Worker via `import(url)`. The main thread injects a `MiddlewareConfig` array — an ordered list of middleware entries — through the same dependency injection pattern used for kernel workers.
...
export const myMiddleware = createKernelMiddleware({
  name: 'MyMiddleware',
  version: '1.0.0',
...
Add a `?url` import and entry to `defaultMiddlewareConfig` in `apps/ui/app/constants/kernel.constants.ts`:
...
```typescript
import { defaultMiddlewareConfig } from '#constants/kernel.constants.js';
import type { MiddlewareConfig } from '@taucad/types';

const extendedConfig: MiddlewareConfig = [
...
Resolution order: `MiddlewareEntry.enabled` > `createKernelMiddleware({ enabled })` > `true`
...
export const debugMiddleware = createKernelMiddleware({
