...
import type { CreateGeometryResult, ExportGeometryResult, GetParametersResult } from '@taucad/kernels';
...
#### `exportGeometry(fileType, tessellation?)`
...
<BuildProvider buildId={id} kernelOptions={extendedConfig}>
```
...
Add a `KernelModules` entry to `cadKernelConfigs` with:
...
Middleware is **dynamically loaded** in the Web Worker via `import(url)`. The main thread injects a `MiddlewareEntries` array — an ordered list of middleware entries — through the same dependency injection pattern used for kernel workers.
...
Add a `?url` import and entry to `defaultMiddlewareEntries` in `apps/ui/app/constants/kernel.constants.ts`:

```typescript
import myMiddlewareUrl from '#components/geometry/kernel/middleware/my.middleware.js?url';

export const defaultMiddlewareEntries: MiddlewareEntries = [
...
```typescript
import { defaultMiddlewareEntries } from '#constants/kernel.constants.js';
import type { MiddlewareEntries } from '@taucad/kernels';

const extendedEntries: MiddlewareEntries = [
  ...defaultMiddlewareEntries,
