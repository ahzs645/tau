...
A [KernelPlugin](../api/kernels) describes a CAD engine. Factory functions like `replicad()`, `openscad()`, and `zoo()` return a registration object:
...
- [API: Kernels](../api/kernels) — `defineKernel` and `KernelDefinition`
- [API: Middleware](../api/middleware) — `defineMiddleware` and `KernelMiddleware`
