...
```typescript
import { createKernelClient, fromNodeFS } from '@taucad/runtime';
import { replicad } from '@taucad/runtime/kernels';
import { esbuild } from '@taucad/runtime/bundler';
...
- **Preset with all middleware**: Use `presets.all()` from `@taucad/runtime` to get kernels, middleware, and bundlers. Merge with `filesystem` and `transport`: `createKernelClient({ ...presets.all(), filesystem, transport })`. See [API Reference: Client](/docs/api/client).
