...
Kernel operations return a discriminated union `KernelResult<T>`. Use the `isKernelSuccess` and `isKernelError` type guards to narrow results, and inspect `KernelIssue` for messages, severity, location, and stack frames.
...
Handle success and error results from `render`, `export`, and `getParameters` using type guards and the `KernelIssue` structure. Apply patterns that work for both render and export calls.
...
### 2. Use Type Guards from @taucad/kernels

Import and use `isKernelSuccess` and `isKernelError` to narrow the result:
...
const result = await client.render({
  file: 'main.ts',
  code: `export default function main() { return null; }`,
});

if (isKernelSuccess(result)) {
  console.log(`Rendered ${result.data.geometries.length} geometry`);
...
```typescript
const result = await client.render({ file: 'main.ts', code: userCode });
...
if (isKernelSuccess(result)) {
  ...
  for (const { blob, name } of blobs) {
    // Save blob to disk or trigger download
  }
} else {
...
```typescript
import { createKernelError } from '@taucad/kernels';
