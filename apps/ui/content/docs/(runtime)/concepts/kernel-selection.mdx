...
When you call `render({ file })` or `setFile(file)`, the runtime must choose which kernel handles the file. Extension alone is insufficient: `.ts` files might use Replicad, OpenCASCADE, or JSCAD depending on imports. The selection cascade uses three strategies in order, with caching to avoid repeated work.
...
```mermaid
flowchart TD
    Start["render / setFile"] --> Ext["1. Extension fast path"]
...
In **autonomous rendering mode** (using `setFile`/`setParameters`), the worker's filesystem watch subscription handles cache invalidation automatically when files change. In **inline code mode** (using `client.render()` with `code`), call `client.notifyFileChanged(paths)` to invalidate the selection cache.
...
- **Selection and Worker** -- Selection runs inside the worker. The client sends a render command or `setFile`; the worker selects the kernel before calling `getParameters` or `createGeometry`.
