...
```typescript @ts-nocheck
// renderer.ts — transport is a wired plugin callable supplied by your app
// package (Electron example: `electronUtilityTransport({ port })` from an
// app-local module built with defineRuntimeTransport).
import { createRuntimeClient, presets } from '@taucad/runtime';
...
// main / utility host — consumes a RuntimeTransportHost from nodeWorkerHost
// or electronUtilityHost (see `@taucad/runtime/host` + examples/electron-tau).
import { createRuntimeHost } from '@taucad/runtime/host';
import { nodeWorkerHost } from '@taucad/runtime/transport/node-worker-host';

const host = createRuntimeHost({
  transport: nodeWorkerHost({ worker: kernelWorker }),
});
```
