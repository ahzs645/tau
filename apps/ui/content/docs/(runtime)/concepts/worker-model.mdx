...
The transport defaults to its bundled worker entry; pass an explicit `url:` only when hosting a custom worker module that composes `KernelRuntimeWorker` with `webWorkerTransport.host` directly.
...
// inferred:
//   transport: inProcessTransport.client({ fileSystem: fromNodeFs('/path/to/project') }),
...
const client = createRuntimeClient({
  ...presets.all(),
  transport: defineRuntimeTransport({ port }),
...
const host = createRuntimeHost({
  ...
  transport: nodeWorkerTransport.host({ url: kernelWorkerEntryUrl }),
