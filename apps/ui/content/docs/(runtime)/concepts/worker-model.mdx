...
```mermaid
flowchart TB
    subgraph Main["Main Thread (display + user input)"]
        Editor["Editor / App"]
    ...
    subgraph FSWorker["FileSystem"]
        FS["FileSystem Backend"]
    end

    Editor -->|"setFile / setParameters"| Client
    Client -->|"MessagePort + SharedArrayBuffer"| RenderLoop
    RenderLoop --> KernelWorker
    KernelWorker --> Middleware
    Middleware --> Kernels
    Kernels -.->|"geometry events"| Scene
    FS -.->|"watch events"| WatchSub
    WatchSub -->|"file changed"| RenderLoop
```
...
- **FileSystem and Worker** -- The filesystem is bridged via MessagePort. In autonomous mode, the worker subscribes to file change events scoped to the current file's dependency graph.
