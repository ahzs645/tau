...
1. The active [transport](../api/transport) raises the abort generation via `signalAbort('superseded')` **before** posting the wire-format command.
...
The transport-owned signal channel was shrunk in v5 to two `Int32` slots — every other worker-to-main signal flows through the single ordered `postMessage` event channel. The slot layout is `@internal` to the transport implementations; consumers of `RuntimeTransport` only see the `signalAbort(reason)` API:
...
The runtime architecture exposes three orthogonal planes — **transport** (the wire), **runner** (where kernel code executes), and **filesystem** (the source of truth for files). Each topology resolves to one config object on each side; the table below shows the canonical recipes that ship today (T1, T2) plus the Phase 1 / Phase 2 targets (T3, T4) that the same primitives unlock without core changes. Every variation in [Architecture](./architecture) is a re-combination of these three planes.
...
import { webWorkerRunner, getDefaultKernelWorkerUrl } from '@taucad/runtime/runner';
import { fromWorker } from '@taucad/runtime/filesystem';

const fmWorker = new Worker(new URL('./fm-worker.ts', import.meta.url), { type: 'module' });

const client = createRuntimeClient({
  ...presets.all(),
  transport: createWorkerTransport(getDefaultKernelWorkerUrl()),
  runner: webWorkerRunner({ url: getDefaultKernelWorkerUrl() }),
  fileSystem: fromWorker(fmWorker),
});
```
...
const client = createRuntimeClient({
  ...
  runner: hostRunner(),
  // No fileSystem on the renderer — the host owns it (TR9).
});
...
import { lruCache } from '@taucad/runtime/cache';
...
A topology is `{ transport, runner, fileSystem }`. The transport knows about the wire. The runner knows about execution location. The FS service knows about authority. Mixing them — for example a `'sharedWorkerTransport'` that also assumes a same-isolate FS — produces N transports instead of `N + M + K` plug points and silently regresses cross-process topologies. T1–T4 share the same primitives; only the bindings change.
