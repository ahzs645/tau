...
- **Preset with all middleware**: Use `presets.all()` from `@taucad/runtime` to get kernels, middleware, and bundlers. Merge with `filesystem` and `transport`: `createRuntimeClient({ ...presets.all(), filesystem, transport })`. See [API Reference: Client](/docs/api/client).
...
- [API Reference: Client](/docs/api/client) -- createRuntimeClient options
