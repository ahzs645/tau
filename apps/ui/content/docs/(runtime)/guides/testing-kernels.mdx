...
```typescript @ts-nocheck
import { createMockLogger, createMockFileSystem, createMockRuntime } from '@taucad/runtime/testing';
...
`createMockFileSystem` returns a `RuntimeFileSystem` with a `mocks` property for vitest assertions.
...
```typescript @ts-nocheck
import { validateGlbData, getInspectReport, extractGltfFromResult } from '@taucad/runtime/testing';
