...
Use `defineKernel` with a definition object. The generic parameters are `<Context, NativeHandle, Options>`. You must implement `initialize`, `getDependencies`, `getParameters`, `createGeometry`, and `exportGeometry`. Optionally implement `canHandle` for kernel selection and `cleanup` for teardown:
