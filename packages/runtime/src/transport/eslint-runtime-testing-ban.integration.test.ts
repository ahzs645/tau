...
const fixturesDir = join(workspaceRoot, 'tools/eslint-fixtures/runtime-testing-ban');

describe('eslint runtime/testing import ban', () => {
  it('flags non-test typescript that imports `@taucad/runtime/testing`', () => {
    expect(() =>
      execFileSync(process.execPath, [eslintCli, join(fixturesDir, 'bad-prod-import.ts'), '--max-warnings', '0'], {
        cwd: workspaceRoot,
        stdio: 'pipe',
      }),
    ).toThrow();
  ...
  it('allows *.test.ts files that import `@taucad/runtime/testing`', () => {
    execFileSync(process.execPath, [eslintCli, join(fixturesDir, 'fine.test.ts'), '--max-warnings', '0'], {
      cwd: workspaceRoot,
      stdio: 'pipe',
    });
