...
Editor writes file
  ...
  → KernelClient.render({ changedPaths }) [Main Thread]
  → KernelWorkerClient.notifyFileChanged() [Main Thread → Worker]
...
The kernel worker becomes an **autonomous reactive render service**. Like a Language Server in LSP, it watches its dependencies, debounces changes, renders, and pushes results -- without the main thread telling it when to act.
...
│                                                                   │
│  Editor ─── setFile / setParameters ──▶ KernelClient              │
│  Params UI ┘                             │    ▲                   │
│                                     (1) Atomics.store            │
│                                     (2) postMessage              │
...
| ---------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SharedArrayBuffer(4)` | Allocated by `KernelClient`, transferred to worker at connect time | Abort generation counter. Main thread writes via `Atomics.store` before posting `setFile`/`setParameters`. Worker's OC Proxy reads via `Atomics.load` at each WASM call boundary. |
...
`SharedArrayBuffer` solves this. It provides a memory region visible to both the main thread and the kernel worker simultaneously:
...
1. Main thread calls `setFile()` or `setParameters()` on `KernelClient`.
2. `KernelClient` writes `Atomics.store(abortFlag, 0, newGeneration)` **before** posting the message.
...
**Before:** cadMachine (~770 lines) + kernelMachine (~630 lines) = ~1400 lines across two machines. cadMachine manages render orchestration, debounce timers, changedPaths accumulation, timeout handling, and 7 states. kernelMachine manages KernelClient lifecycle and forwards events between client and cadMachine.

**After:** Single unified machine, ~150 lines. Handles KernelClient lifecycle (connect, subscribe, terminate) and reflects worker-reported state:
...
lifecycle: connecting invokes KernelClient creation, idle/rendering/error reflect worker state
```

No `bufferingFile`, `bufferingParameters`, `createGeometry`, `changedPaths`, `isDifferentFile`, `renderTimeout`, and no separate kernelMachine. The worker handles scheduling and debounce; cadMachine handles KernelClient lifecycle and display state.

### kernelMachine → eliminated

**Before:** ~630 lines. Manages KernelClient lifecycle, forwards createGeometry to client.render(), forwards events between client and cadMachine. Acts as a middleman that adds indirection without independent decision-making.

**After:** Eliminated entirely. Its responsibilities collapse into cadMachine:

- **KernelClient creation and connection** -- handled by a promise actor invoked from cadMachine's `connecting` state.
- **Event forwarding** -- cadMachine subscribes directly to `KernelClient.on(...)` events. No intermediate machine needed.
- **Lifecycle cleanup** -- cadMachine's `exit` action calls `client.terminate()`.

The current kernelMachine exists because the old protocol required orchestrating `createGeometry` → `render()` → result forwarding. With the autonomous worker model, there is no render command to orchestrate -- the worker self-renders. The only commands cadMachine sends are `setFile`, `setParameters`, and `export`, which map directly to `KernelClient` methods. An intermediate machine adds no value.
...
```typescript
const client = createKernelClient({ kernels, middleware, bundlers });
...
The kernel worker is essentially a "geometry server" following the same architectural pattern that powers every modern code editor.
...
3. Add SharedArrayBuffer abort channel: allocate in `KernelClient`, transfer at connect, wire `Atomics.store` into `setFile`/`setParameters`.
...
9. Collapse `kernelMachine` into `cadMachine` -- move KernelClient lifecycle (creation, connection, event subscription, cleanup) into cadMachine as a `connecting` state with a promise actor.
...
12. Update `KernelClient` API to reactive event emitter pattern.
