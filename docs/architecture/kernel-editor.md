...
**Reference** -- documents the reactive integration between the filesystem layer, kernel workers, and the editor UI. Companion to [kernel-topology.md](kernel-topology.md) (autonomous render service) and [filesystem-policy.md](../policy/filesystem-policy.md) (implementation rules).
...
A compilation unit is a single `cadMachine` actor managing one kernel worker for one entry file:

```
buildMachine spawns cadMachine(entryFile, kernelType)
  → cadMachine enters 'connecting' state
  → creates KernelClient, connects to kernel worker
  ...
  [build closes]
  → cadMachine disposes KernelClient
