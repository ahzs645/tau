...
This unlocks four product capabilities at once: third-party kernels (CadQuery-via-Pyodide, Brep-CAD, custom DSLs); user-installable test runtimes (`@taucad/test-runtime` becomes a normal middleware install per [`browser-first-parameter-aware-testing.md`](./browser-first-parameter-aware-testing.md)); "bring your own observability" (custom telemetry middleware); and the Parts Registry [`tau-parts-registry-and-marketplace.md`](./tau-parts-registry-and-marketplace.md) where parts can ship with their own renderer adapters. The security model is the same as today's user code — everything runs in the kernel worker, which has no DOM access, no cross-origin network access (subject to COEP), no chat/auth credentials. Sandboxing is by **execution context**, not by code review.
...
- Sibling blueprint: [`docs/research/browser-first-parameter-aware-testing.md`](./browser-first-parameter-aware-testing.md) — `@taucad/test-runtime` becomes a normal middleware install.
