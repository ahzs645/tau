 ...
 */
function createTabletop(p = defaultParams) {
 ...
 */
function createLeg(p: typeof defaultParams, x: number, y: number) {
 ...
 */
function createApron(p = defaultParams) {
  if (!p.includeApron) {
    return null;
  }
  ...
  const backApron = draw([
    ...
    legYPos + legHalfWidth - p.apronThickness,
  ])
    .hLine(2 * legXPos + p.legWidth)
    .vLine(p.apronThickness)
    .hLine(-(2 * legXPos + p.legWidth))
  ...
  const rightApron = draw([
    legXPos + legHalfWidth - p.apronThickness,
    -legYPos - legHalfWidth + p.apronThickness,
  ])
    .hLine(p.apronThickness)
    .vLine(2 * legYPos + p.legWidth - p.apronThickness)
 ...
 */
function createShelf(p = defaultParams) {
  if (!p.includeShelf) {
    return null;
  }
  ...
  if (p.roundedCorners) {
    const shelfRadius = Math.min(
      p.cornerRadius * 0.8,
      Math.min(shelfWidth, shelfLength) * 0.2,
    );
