...
export default function main(p = defaultParams): Shape3D {
  ...
  if (p.backrestAngle !== 0) {
    ...
    const rotationAxisOrigin: [number, number, number] = [
      0,
      -p.seatDepth / 2,
      p.seatHeight,
    ];
    const rotationAxisDirection: [number, number, number] = [1, 0, 0]; // Rotate around X-axis
  ...
  if (p.filletRadius > 0) {
    ...
    try {
      chair = chair.fillet(p.filletRadius, (edgeFinder) =>
        edgeFinder.either([
          (edgeFinder) => edgeFinder.inPlane('XY', p.seatHeight),
          (edgeFinder) =>
            edgeFinder.containsPoint([
              0,
              -p.seatDepth / 2,
              p.seatHeight + p.backrestHeight * 0.9,
            ]),
        ]),
      );
    } catch (error) {
