...
- Policy docs live in `docs/policy/` (testing, library-api, vision, lint, xstate, typescript, filesystem, react-testing, commit, agents-md, context-engineering, jsdoc, diagram, ui, accessibility, and more); research docs in `docs/research/` (including `rpc-best-practices.md`, `observability-architecture.md`); architecture docs in `docs/architecture/` (e.g., `runtime-topology.md`) serve as binding contracts for implementation
...
- External repos in `repos/` managed via `repos.yaml` and `pnpm repos`; gitignored and cursorignored; add to `.oxlintrc.json` ignorePatterns; `repos/opencascade.js` WASM build uses `BUILTIN_ADDITIONAL_BIND_CODE` (Python layer), full builds (10-30+ min) use `nohup`; `repos/vscode` used as architectural reference for FS events, IPC, caching, and binary transfer patterns
...
- API deployed to Fly.io (`apps/api/fly.prod.toml`); OTEL SDK bootstraps in `apps/api/app/telemetry/otel.ts` (imported first in `main.ts`); `@fastify/otel` for HTTP tracing; `PrometheusExporter` on port 9464 scraped by Fly.io's managed VictoriaMetrics; traces+logs exported via OTLP/HTTP to Grafana Cloud; `TelemetryModule` (`@Global`) provides `TracerService` + `@Span()` decorator; metrics defined in `apps/api/app/telemetry/metrics.ts`; `prom-client` fully replaced by `@opentelemetry/api`
- Grafana dashboards-as-code in `infra/grafana/` with provisioning configs and 6 dashboard JSONs (API, WS, AI, Kernel, Infra, SLO); `grafana/otel-lgtm` in `docker-compose.yml` provides local LGTM stack parity; alert rules in `infra/grafana/alerts/`; `pnpm grafana:up|down|logs` scripts
- Editor architecture: machine owns openFiles, ref-counting, force-close; dockview subscribes only; use unique panel IDs (not file path); single FS worker — all filesystem access flows through one serialized worker with ZenFS and IndexedDB backend; `fromSafeAsync` (`#lib/xstate.lib.js`) replaces `fromPromise` for all UI XState async actors
